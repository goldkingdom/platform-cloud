 ...
 */
@Service
public class MysqlPagerServiceImpl implements PagerService {
    ...
    public List query(StringBuffer sql, Map params, Map pager, String countColumn) {
        ...
        //根据分页信息进行分页查询
        StringBuffer pagerSql = sql.append(" limit {offset,Integer},{pageSize,Integer}");
        params = params == null ? Maps.newHashMap() : params;
        params.put("offset", (ConvertUtil.objectToInt(pager.get("currentPage")) - 1) * ConvertUtil.objectToInt(pager.get("itemsPerPage")));
        params.put("pageSize", pager.get("itemsPerPage"));
        List<Map> list = sqlService.query(pagerSql, params);
        str = null;
        return list;
    }
